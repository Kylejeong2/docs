...
Harness profiles are complementary to [Provider profiles](/oss/python/deepagents/models#provider-profiles): provider profiles shape how the model is built, while harness profiles shape how the harness works once that model is in use.
