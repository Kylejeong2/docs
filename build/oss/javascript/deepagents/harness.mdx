...
When you pass a preconfigured chat model instance ([`BaseChatModel`](https://reference.langchain.com/javascript/langchain-core/language_models/chat_models/BaseChatModel) subclass) instead of a `provider:model` string, the harness synthesizes the canonical `provider:identifier` key from the instance and looks it up in this order: `provider:identifier` -> identifier-only (only when the identifier already contains `:`) -> provider-only fallback.

Harness profiles are complementary to [Provider profiles](/oss/javascript/deepagents/models#provider-profiles): provider profiles shape how the model is built, while harness profiles shape how the harness works once that model is in use.
