...
A `ProviderProfile` declares how Deep Agents should construct a chat model for a given provider or specific model spec. It applies only when the harness turns a `provider:model` string into a client via [`init_chat_model`](https://reference.langchain.com/javascript/langchain/chat_models/universal/initChatModel) (not when you pass a preconfigured instance):
